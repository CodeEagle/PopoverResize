...
public class PopoverResize: NSPopover {
    ...
    public init(min: NSSize, max: NSSize, heightFromBottom: CGFloat = 0) {
        ...
        if  self.max.width == 0 {
            self.max.width = CGFloat(NSScreen.screens[0].frame.width - 110)
        }
...
//        print("MOVE x: \(movedX), y: \(movedY)")
    ...
    private static func getCursor(_ name: String) -> NSCursor {
        ...
        let imagePath = path + "/Contents/Resources/PopoverResize_PopoverResize.bundle/Contents/Resources/resources/\(name)_cursor.pdf"
        let infoPath = path + "/Contents/Resources/PopoverResize_PopoverResize.bundle/Contents/Resources/resources/\(name)_info.plist"
